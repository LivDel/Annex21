...
@Injectable()
export class BillingServiceCore {
  private readonly logger = new Logger(BillingServiceCore.name);
  private stripe: Stripe | null = null;

  constructor(@Inject(DOMAIN_STORE) protected readonly store: DomainStore) {}
  ...
  protected async dispatchEvent(event: Stripe.Event): Promise<void> {
    switch (event.type) {
  ...
  private async onSubscriptionDeleted(sub: Stripe.Subscription): Promise<void> {
    ...
    if (!orgId) return;
    // Clear subscription id → hasSubscription false (canceled→empty soft UI)
    await this.applyStatus(
      orgId,
      'canceled',
